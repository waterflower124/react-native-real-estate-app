...
export default class VerifyPhoneScreen extends Component {
    ...
    resendPhoneNumber = () => {
        try {
            ...
            params.append("phone", this.state.data.phone);
    ...
    user_signup = async() => {
        try {
            ...
            let uri = Global.BASE_URL + "/api/register";
            let params = new FormData();
            params.append("name", this.state.data.name);
            params.append("person_id", this.state.data.person_id);
            params.append("phone", this.state.data.phone);
            params.append("password", this.state.data.password);
            params.append("type", this.state.data.type);
            params.append("otp", this.state.pin_code);
                                                
            console.log(TAG + " callSignUpAPI uri " + uri);
            console.log(TAG + " callSignUpAPI params " + JSON.stringify(params));

            WebService.callServicePost(uri, params, this.handleSignUpAPI);
        } catch (error) {
            console.log(error)
            this.setState({
                loading: false,
    ...
    handleSignUpAPI = async(response, isError) => {
        console.log(TAG + " callSignUpAPI Response " + JSON.stringify(response));
        console.log(TAG + " callSignUpAPI isError " + isError);
        try {
            if(!isError) {
                if(response.status == "success") {
            ...
            <View style = {{flex: 1, backgroundColor: Colors.main}}>
            {
                ...
                <View style = {{flex: 1, width: '100%'}}>
                    <View style = {{width: '100%', marginVertical: 20, alignItems: 'center'}}>
                        <Text style = {[{fontSize: 32, color: Colors.white}, stylesGlobal.font_GESSTwoMedium_Medium]}>{strings.verify_phone}</Text>
                    </View>
