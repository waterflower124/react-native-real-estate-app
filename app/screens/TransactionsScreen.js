...
export default class TransactionsScreen extends Component {
    ...
    getTransactions = async() => {
        try {
            ...
            var uri = Global.BASE_URL + "/api/transactions";
            
            console.log(TAG + " callGetTransactionsAPI uri " + uri);

            WebService.callServiceGet(uri, this.handleGetTransactionsAPI);
        } catch (error) {
            console.log(error)
            this.setState({
                loading: false,
    ...
    handleGetTransactionsAPI = async(response, isError) => {
        console.log(TAG + " callGetTransactionsAPI Response " + JSON.stringify(response));
        console.log(TAG + " callGetTransactionsAPI isError " + isError);
        try {
            if(!isError) {
    ...
    displayPDF = (item) => {
        ...
        var filename = "transaction_" + item.date + ".pdf";
        ...
        //     loading: false
        // })
        this.props.navigation.navigate("DisplayPDFScreen", {path: downloadDest, filename: filename, url: item.transaction_pdf})
    }

    renderHeader = () => {
        return (
            <View style = {{width: '100%', height: 30, flexDirection: 'row', borderWidth: 1, borderColor: Colors.text_main_color}}>
                <View style = {{width: '40%', height: '100%', justifyContent: 'center', alignItems: 'center', borderEndWidth: 1, borderEndColor: Colors.text_main_color}}>
                    <Text style = {[{fontSize: 14, color: Colors.text_main_color,}, stylesGlobal.font_GESSTwoMedium_Medium]}>{strings.title_transaction}</Text>
                </View>
                <View style = {{width: '30%', height: '100%', justifyContent: 'center', alignItems: 'center', borderEndWidth: 1, borderEndColor: Colors.text_main_color}}>
                    <Text style = {[{fontSize: 14, color: Colors.text_main_color,}, stylesGlobal.font_GESSTwoMedium_Medium]}>{strings.title_amount}</Text>
                </View>
                <View style = {{width: '30%', height: '100%', justifyContent: 'center', alignItems: 'center', borderEndWidth: 1, borderEndColor: Colors.text_main_color}}>
                    <Text style = {[{fontSize: 14, color: Colors.text_main_color,}, stylesGlobal.font_GESSTwoMedium_Medium]}>{strings.title_date}</Text>
                </View>
            </View>
        )
    }
    ...
    renderBill = (item, index) => {
        return (
            <View key = {index} style = {{width: '100%', height: 40, flexDirection: 'row', borderStartWidth: 1, borderBottomWidth: 1, borderEndWidth: 1, borderColor: Colors.text_main_color}}>
                <View style = {{width: '40%', height: '100%', justifyContent: 'center', alignItems: 'center', borderEndWidth: 1, borderEndColor: Colors.text_main_color}}>
                    <TouchableOpacity style = {{width: '80%', height: '80%'}} onPress = {() => this.displayPDF(item)}>
                ...
                <View style = {{width: '30%', height: '100%', justifyContent: 'center', alignItems: 'center', borderEndWidth: 1, borderEndColor: Colors.text_main_color}}>
                {
                    ...
                    <NumberFormat value={item.amount} displayType={'text'} thousandSeparator={true} renderText={value => <Text style = {[{fontSize: 12, color: Colors.text_main_color,}, stylesGlobal.font_GESndBook]}> {value}</Text>} />
                }
                {
                    !(item.amount != null && item.amount != "") &&
                    <Text style = {[{fontSize: 12, color: Colors.text_main_color,}, stylesGlobal.font_GESndBook]}> {"0"}</Text>
                }
                </View>
                <View style = {{width: '30%', height: '100%', justifyContent: 'center', alignItems: 'center', borderEndWidth: 1, borderEndColor: Colors.text_main_color}}>
                    <Text style = {[{fontSize: 12, color: Colors.text_main_color,}, stylesGlobal.font_GESndBook]}>{item.date}</Text>
                </View>
            </View>
        )
    }
        ...
        return (
            <SafeAreaView style = {{flex: 1, backgroundColor: Colors.main_bg_color, justifyContent: 'center', alignItems: 'center'}}>
            {
                ...
                <View style = {{width: '100%', height: 40, backgroundColor: Colors.main, paddingHorizontal: 15, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <Text style = {[{fontSize: 16, color: Colors.main_bg_color,}, stylesGlobal.font_GESSTwoMedium_Medium]}>{strings.menu_transactions}</Text>
                ...
                <View style = {{flex:1 , width: '100%', paddingHorizontal: 10, marginVertical: 15}}>
                    <FlatList
                        ...
                        onRefresh={() => this.setState({refreshing: true}, () => this.getTransactions())}
                        refreshing={this.state.refreshing}
                        data={this.state.transaction_array}
